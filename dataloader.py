...
def collate_fn_mfr(list_data):
    for ind, (a, b, c) in enumerate(list_data):
        return a, b, c
...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000):
    ...
    for i in range(len(dataset)):
        timer.tic()
        batched_input = collate_fn([dataset[i]], config, neighborhood_limits=[hist_n] * 6)
        # update histogram
...
def get_dataloader_fmr(dataset, batch_size=1, num_workers=4, shuffle=True):
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=shuffle,
        num_workers=num_workers,
        # https://discuss.pytorch.org/t/supplying-arguments-to-collate-fn/25754/4
        collate_fn=partial(collate_fn_mfr),
        drop_last=False
    )
    return dataloader
